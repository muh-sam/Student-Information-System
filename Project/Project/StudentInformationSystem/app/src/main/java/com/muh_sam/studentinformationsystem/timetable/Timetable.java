...
public class Timetable extends ActionBarActivity implements TimetableFragment.CallBack {
    ...
    public void onItemSelected(int TimetableId) {
        if(mTwoPane)
        {
            ...
            args.putString(TimetableDetailActivity.TIMETABLE_KEY, String.valueOf(TimetableId));
            TimetableDetailFragment detailFragment = new TimetableDetailFragment();
            detailFragment.setArguments(args);
            getSupportFragmentManager().beginTransaction().replace(R.id.timetable_detail_container, detailFragment).commit();
        }else
        {
            Intent intent = new Intent(this, TimetableDetailActivity.class).putExtra(TimetableDetailActivity.TIMETABLE_KEY, String.valueOf(TimetableId));
